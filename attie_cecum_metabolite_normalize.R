################################################################################
# Normalize and impute missing data in the Attie cecum metabolite data set.
# Daniel Gatti
# dan.gatti@jax.org
# Nov 15, 2017
################################################################################
...
metab = read_delim(paste0(input.dir, "formatted_data/FilterdbyR_DOCecumMetbolites_BatchandCovariatesAppended.txt"),
        delim = "\t")
...
pdf("figures/cecum_metabolites_unnormalized_all_data_PCA.pdf")
batch.colors = as.numeric(factor(annot$batch))
plot(scores(pc.data), pch = 16, col = 0, main = "Un-normalized Cecum Metabolites, Colored by Batch")
...
pdf("figures/cecum_metabolites_unnormalized_PCA.pdf")
...
diet.days = factor(annot$diet.days, levels = sort(unique(annot$diet.days)))
...
saveRDS(data.rz, file = paste0(output.dir, "attie_cecum_metabolites_zscore_normalized.rds"))


# Make PCA plots of the normalized data, colored by batch, sex, etc.
pdf("figures/cecum_metabolites_normalized_PCA.pdf", width = 12, height = 7)
...
pdf("figures/cecum_metabolites_normalized_boxplot.pdf", width = 12, height = 7)
boxplot(data, range = 0)
dev.off()

pdf("figures/cecum_metabolites_normalized_heatmap.pdf", width = 12, height = 12)
